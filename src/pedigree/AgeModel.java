...
public class AgeModel {
    ...
    public static void main(String[] args) {
        String[] arg = {"0.01", "0.05", ".9"};
        int arg_idx = 0;
        double acc = Double.parseDouble(arg[arg_idx++]);
        double dth = Double.parseDouble(arg[arg_idx++]);
        double scale = DEFAULT_SCALE;

        if (arg_idx < arg.length)
            scale = Double.parseDouble(arg[arg_idx++]);
